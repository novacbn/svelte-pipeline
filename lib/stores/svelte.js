...
function PipelineSvelteOptions(options = {}) {
  const {compiler: compiler2 = {}, context = {}, imports = {}, mode = pipeline.PIPELINE_MODES.evaluate} = options;
...
function pipeline_svelte(options) {
  ...
  const derived_store = derived(writable_store, (script) => {
    ...
    if (mode === pipeline.PIPELINE_MODES.validate) {
      return {type: pipeline.PIPELINE_RESULT_TYPES.validated};
    }
    const {css, js} = compiler.compile(script, compiler2);
    [validated, message] = pipeline.validate_code(js.code);
    if (!validated)
      return {message, type: pipeline.PIPELINE_RESULT_TYPES.error};
    const [evaluated, module2] = pipeline.evaluate_code(js.code, context);
    if (!evaluated) {
      return {message: module2, type: pipeline.PIPELINE_RESULT_TYPES.error};
    }
    return {
      module: module2,
      stylesheet: css.code,
      type: pipeline.PIPELINE_RESULT_TYPES.evaluated
    };
  });
  return {
